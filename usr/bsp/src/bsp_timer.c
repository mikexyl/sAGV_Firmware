...
static uint8_t timer_type[4] = {TIMER_TYPE_ONESHOT};
#if 0
...
*             _pCallBack : 定时时间到后，被执行的函数 *	返 回 值: 无 *********************************************************************************************************
*/
void bsp_StartHardTimer(uint8_t _CC, uint8_t type, uint32_t _uiTimeOut, void * _pCallBack)
{
    uint32_t cnt_now;
    uint32_t cnt_tar;
...
{
		TIM_ITConfig(TIM_HARD, _CC, DISABLE);	/* 使能CC4中断 */
}
...
void TIM5_IRQHandler(void)
{
    if (TIM_GetITStatus(TIM_HARD, TIM_IT_CC1))
    {
        ...
        if(timer_type[0] == TIMER_TYPE_ONESHOT)
    ...
    {
        TIM_ClearITPendingBit(TIM_HARD, TIM_IT_CC2);
        if(timer_type[1] == TIMER_TYPE_ONESHOT)
    ...
    {
        TIM_ClearITPendingBit(TIM_HARD, TIM_IT_CC3);
        if(timer_type[2] == TIMER_TYPE_ONESHOT)
    ...
    {
        TIM_ClearITPendingBit(TIM_HARD, TIM_IT_CC4);
        if(timer_type[3] == TIMER_TYPE_ONESHOT)

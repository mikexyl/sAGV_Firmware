 ...
 *
 *  @brief : This file contains example application for BT Classic powersave modes in SPP Slave Role
 *
 *  @section Description  This application serves power save profiles in BT classic.
 *
 */
...
#define LOG_PRINT
#endif
#define RSI_BT_LOCAL_NAME             "BT_SPP_POWER_SAVE"
...
void rsi_bt_on_ssp_complete (uint16_t resp_status, rsi_bt_event_ssp_complete_t *ssp_complete)
{
  ...
  LOG_PRINT ("ssp_complete: str_conn_bd_addr: %s\r\n",str_conn_bd_addr);
	LOG_PRINT ("ssp_complete: str_conn_bd_addr: %d\r\n",resp_status);
}

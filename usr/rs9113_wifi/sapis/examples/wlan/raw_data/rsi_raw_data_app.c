 ...
 *
 *  @brief : This file contains example application for sending ping response via raw data in AP mode
 *
 *  @section Description  This file contains example application for sending ping response via raw data in AP mode 
 *
 */
...
void raw_data_receive_handler(uint16_t status, const uint8_t *data, const uint16_t length)
{
...
#endif
    {
      status = rsi_send_raw_data(&send_buffer[0],framelen);
      if(status != RSI_SUCCESS)
      {
        return status;
      }

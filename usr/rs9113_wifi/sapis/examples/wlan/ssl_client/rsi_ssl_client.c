 ...
 *
 *  @brief : This file contains example application for TCP client socket over SSL
 *
 *  @section Description  This file contains example application for TCP client socket  over SSL
 *
 *
 */
...
int32_t rsi_ssl_client()
{
  ...
  //! Create socket
#if SSL
  client_socket = socket(AF_INET, SOCK_STREAM, 1);
#else
  ...
  {
    //! Send data on socket
    status = send(client_socket,(int8_t *)"Hello from SSL TCP client!!!", (sizeof("Hello from SSL TCP client!!!") - 1), 0);
    if(status < 0)
    {
      status = rsi_wlan_get_status();
      shutdown(client_socket, 0);

/**
 * @file    : firmware_upgrade_ota_server.c
 * @version : 0.4
 * @date    : 19 May 2016
 *
 ...
 *
 *  @brief : This file contains TCP server application which sends firmware file to remote peer
 *
 *  @section Description  This file contains TCP server application which sends firmware file to remote peer
 *
 *
 */
...
again:	
    ...
    if(ret_len>0)
    {
      ...
      else if(cmd_type == RPS_DATA )
      {
        ...
        {
          fseek(fp, 0, SEEK_SET);
          fseek(fp, (32 + (800 * (chunk_no-2))), SEEK_CUR);
        }

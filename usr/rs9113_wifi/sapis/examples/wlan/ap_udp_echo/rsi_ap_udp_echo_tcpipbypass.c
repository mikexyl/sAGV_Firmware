 ...
 *
 *  @brief : This file contains wlan related operations to create ap and to perform a UDP echo chat with its connected client
 *
...
extern void rsi_packet_receive_notify_handler(uint16_t status, const uint8_t *buffer, const uint16_t length);
...
void rsi_packet_receive_notify_handler(uint16_t status, const uint8_t *buffer, const uint16_t length)
{

 ...
 *
 *  @brief : This file contains example appication for multicast socket
 *
 *  @section Description  This file contains example application for join to multicast group and 
 *  receive data on udp socket on multicast group and then leaving multicast group
 *
 *
 */
  ...
  while(packet_count < NUMBER_OF_PACKETS)
    {
        ...
        //! Send data on socket
        status = sendto(server_socket, (int8_t *)"Hello from UDP client!!!", (sizeof("Hello from UDP client!!!") - 1), 0, (struct sockaddr *)&client_addr, sizeof(client_addr));
        if(status < 0)
        {
          status = rsi_wlan_get_status();
          shutdown(server_socket, 0);

 ...
 *
 *  @section Description  This application used to set the immediate alert service
...
#define  RSIBLE_REMOTE_BD_ADDRESS   "00:23:A7:7D:07:72"
...
void rsi_ble_immediate_alert_client (void)
{
  ...
  status = rsi_device_init(RSI_LOAD_IMAGE_I_FW);
	...
	//! Set local name
	rsi_bt_set_local_name((int8_t *)RSI_BLE_APP_NAME);

	//! initiating connection with remote device
	rsi_ble_connect ((int8_t)0, (int8_t *)RSIBLE_REMOTE_BD_ADDRESS);
	...
	while (1) {
		...
		switch (event_id) {
			...
			case RSI_BLE_CONN_EVENT:
			{
				...
				rsi_ble_get_profile_async (remote_dev_addr, service_uuid, &rsi_ble_service);
			}
			break;
			...
			case RSI_BLE_GET_RSSI:
			{
				...
				if (rsi_ble_att1_val_hndl != 0) {
					...
					rsi_6byte_dev_address_to_ascii ((int8_t *)remote_dev_addr, (uint8_t *)conn_event_to_app.dev_addr);
					rsi_bt_get_rssi ((int8_t *)remote_dev_addr, (uint8_t *)&rssi);

					if (rssi >= RSI_BLE_RSSI_THRESHOLD_VALUE ) {
						if (data == 0) {
							data = 1;
							//! set the middle alert at GATT server
							rsi_ble_set_att_cmd (remote_dev_addr, rsi_ble_att1_val_hndl, 1, &data);
						}
					} else {
						if (data == 1) {
							data = 0;

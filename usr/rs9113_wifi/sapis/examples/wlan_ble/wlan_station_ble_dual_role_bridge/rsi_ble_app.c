...
{
  rsi_6byte_dev_address_to_ascii ((int8_t *)remote_dev_addr, (uint8_t *)resp_conn->dev_addr);	
...
static void rsi_ble_on_disconnect_event(rsi_ble_event_disconnect_t *resp_disconnect, uint16_t reason)
{
  ...
  rsi_6byte_dev_address_to_ascii ((int8_t *)remote_dev_addr, (uint8_t *)resp_disconnect->dev_addr);	
...
uint32_t rsi_ble_app_init (void)
{
  ...
  rsi_6byte_dev_address_to_ascii ((int8_t *)str_loal_addr, (uint8_t *)loal_addr);	
 ...
 * @section description
 */
uint32_t rsi_ble_app_task (void)
{
  int32_t status = 0;
  int32_t  event_id;
  int8_t   data[20] = {0};
...
scan1:
        ...
        if(!strcmp(conn_dev_addr, RSI_BLE_DEV_ADDR))
        {
          rsi_ble_app_set_event (RSI_BLE_SLAVE_START_NOTIFY);
        }
...
adv:
    ...
    case RSI_BLE_GATT_WRITE_EVENT:
      ...
      {
        slave_val = 0x0001;
        rsi_ble_set_att_value_async (RSI_BLE_DEV_ADDR, 0x60, 2, (uint8_t *)&slave_val);

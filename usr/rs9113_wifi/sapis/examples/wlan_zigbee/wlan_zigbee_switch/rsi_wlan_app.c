...
#define SERVER_IP_ADDRESS 0x6401A8C0
...
extern int32_t rsi_zigbee_app_send_to_wlan(uint8_t msg_type,uint8_t *buffer, uint32_t length);
...
extern void rsi_join_fail_handler(uint32_t status,const uint8_t *buffer, const uint32_t length);
extern void rsi_ip_renewal_fail_handler(uint32_t status,const uint8_t *buffer, const uint32_t length);
extern void rsi_remote_socket_terminate_handler(uint32_t status,const uint8_t *buffer, const uint32_t length);
extern void rsi_ip_change_notify_handler(uint32_t status,const uint8_t *buffer, const uint32_t length);
...
{
  //! send packet to ble
  rsi_wlan_app_send_to_zigbee(RSI_DATA, buffer, length);
}
...
void rsi_ip_renewal_fail_handler(uint32_t status, const uint8_t *buffer, const uint32_t length)
{
...
void rsi_remote_socket_terminate_handler(uint32_t status, const uint8_t *buffer, const uint32_t length)
{
  //! update wlan appication state
  rsi_wlan_app_cb.state = RSI_WLAN_SOCKET_CONNECTED_STATE;
}
...
void rsi_ip_change_notify_handler(uint32_t status, const uint8_t *buffer, const uint32_t length)
{
  //! update wlan appication state
  rsi_wlan_app_cb.state = RSI_WLAN_SOCKET_CONNECTED_STATE;
}

...
int32_t rsi_web_socket_create(int8_t flags, uint8_t *server_ip_addr, uint16_t server_port, uint16_t device_port,
                                    uint8_t *webs_resource_name, uint8_t *webs_host_name, int32_t *socket_id,                                    
                                    void (*web_socket_data_receive_notify_callback)(uint32_t sock_no, const struct sockaddr*addr, uint8_t *buffer, uint32_t length))
{
  ...
  //! Create socket
  sockID = socket_async(protocolFamily, SOCK_STREAM, protocol, web_socket_data_receive_notify_callback);
  if(sockID < 0)
  {
  ...
  *(int32_t *)socket_id = sockID;
  ...
  if(protocolFamily == AF_INET)
  {
    ...
    //! Bind socket
    status = bind(sockID, (struct sockaddr *) &client_addr, sizeof(client_addr));
  }
  else if(protocolFamily == AF_INET6)
  {
    ...
    //! Bind socket
    status = bind(sockID, (struct sockaddr *) &client_addr_v6, sizeof(client_addr_v6));
  }
...
int32_t rsi_web_socket_send_async(uint32_t sockID, uint8_t opcode, int8_t *msg, int32_t msg_length)
{
  ...
  status = send(sockID, msg, msg_length, 0);

...
int32_t rsi_driver_send_data(uint32_t sockID, uint8_t* buffer, uint32_t length, struct sockaddr *destAddr)
{
  ...
  if(pkt == NULL)
  {
    APP_TRACE("rsi_driver_send_data failed at line:%d\r\n", __LINE__);
  ...
  if(((rsi_socket_pool[sockID].sock_type >> 4) & 0xF) == AF_INET)
  {
    ...
    memcpy((uint8_t *)send->dest_ip_addr.ipv4_address,(uint8_t *) ntohl(&(((struct sockaddr_in *)destAddr)->sin_addr.s_addr)), RSI_IPV4_ADDRESS_LENGTH);

    //! Fill destination port
    rsi_uint16_to_2bytes(send->dest_port, ntohs(((struct sockaddr_in *)destAddr)->sin_port));
  ...
  else
  {
    ...
    //! Fill destination port
    rsi_uint16_to_2bytes(send->dest_port, ntohs(((struct sockaddr_in6 *)destAddr)->sin6_port));
...
int32_t rsi_driver_process_recv_data(rsi_pkt_t *pkt)
{
  ...
  {
    struct sockaddr_in src_addr_inet;
    struct sockaddr_in6 src_addr_inet6;
    struct sockaddr *src_addr;

    if((sock_info->sock_type >> 4) == AF_INET)
    {
        src_addr_inet.sin_family = AF_INET;
        src_addr_inet.sin_port  = rsi_bytes2R_to_uint16(recv->dest_port);
        memcpy(&src_addr_inet.sin_addr.s_addr, recv->dest_ip_addr.ipv4_address, RSI_IPV4_ADDRESS_LENGTH);

        src_addr = (struct sockaddr*)&src_addr_inet;
    }
    else
    {
        src_addr_inet6.sin6_family = AF_INET6;
        src_addr_inet6.sin6_port  = rsi_bytes2R_to_uint16(recv->dest_port);
        memcpy(src_addr_inet6.sin6_addr.s6_addr, recv->dest_ip_addr.ipv6_address, RSI_IPV6_ADDRESS_LENGTH);

        src_addr = (struct sockaddr*)&src_addr_inet6;
    }
    ...
    if(sock_info->sock_receive_callback != NULL)
    {
      if((sock_info->sock_bitmap & RSI_SOCKET_FEAT_WEBS_SUPPORT))
      {
	    *(uint8_t *)(recv + data_offset - 1) = websocket_info;
        ...
        sock_info->sock_receive_callback(sockID, src_addr, ((uint8_t *)recv + data_offset - 1), data_length + 1);
      }
      else
      {
        //! Call registered callback if asynchronous callback
        sock_info->sock_receive_callback(sockID, src_addr, ((uint8_t *)recv + data_offset), data_length);
      }
    }
    return RSI_SUCCESS;
  }

...
#define MAX_MOTION_EVENT_NUM    16
...
static float theta_buffer[8];
static sliding_filter_t det_theta_filter = {8, 0, NULL};
...
int agv_init(uint32_t agvid)
{
    ...
    //sliding_filter_init(&det_theta_filter, theta_buffer);
...
void motionctrl_event_callback(motion_event_t evt, uint32_t len, const uint8_t *params)
{
    ...
    if( (evt == MOTIONCTRL_EVENT_POINT_ARRIVAL)
        && ( (Motionstyle == MOTIONSTATE_GOSTRAIGHT)||(Motionstyle == MOTIONSTATE_GOBACKWARD) ) )
    {
      /*
       * 二维码点校验
       * */
      ...
      if(Motionstyle == MOTIONSTATE_GOSTRAIGHT)
      {
          ...
          if( (fabs(GS_DEV) < 30) && (fabs(GS_Angle) < 2) )
          {
    ...
    if( (evt == MOTIONCTRL_EVENT_ACTION_OVER) && (params[0] == MOTIONSTATE_GOSTRAIGHT) )
    {
        float det;
...
static int process_action_cmd(uint32_t action, uint32_t paramlen, const uint8_t *params)
{
   ...
   */
    ...
    if(ret == -PERR_EBUSY)
    {
        ...
        if(data == NULL)
            return -PERR_ENOMEM;
        if(paramlen > MAX_MOTION_CMD_LEN)
            APP_TRACE("AGV ERROR: cmd params too long. %d\r\n", paramlen);
...
static int get_status(agv_status_t *stat)
{
    ...
    agvstat.battery = bat_info->power;
...
static int issue_next_cmd(uint8_t evt)
{
    ...
    switch(evt)
    {
        ...
        case MOTIONCTRL_EVENT_POINT_ARRIVAL:
            ...
            {
              APP_TRACE("Action is denied, post back.\r\n");
              /*
...
static void agv_task(void *param)
{
    ...
    while(1)
    {
      ...
      if(data != NULL)
      {
          ...
          {
              APP_TRACE("On mcevt %d:  x=%f, y=%f, angle=%f.\r\n", 
...
static int process_motionctrl_event(motion_event_t evt, uint32_t param_l, const uint8_t *param)
{
    ...
    switch(evt)
    {
        ...
        case MOTIONCTRL_EVENT_ERROR:
          ...
          APP_TRACE("agv error: %d\r\n", param[0]);
          if((agvstat.error & (1<<param[0])) != 0)
            return 0;

          agvstat.error |= 1<<param[0];
        ...
        case MOTIONCTRL_EVENT_ERROR_RECOVERY:
          ...
          APP_TRACE("agv error recovery: %d\r\n", param[0]);
          if(agvstat.error == 0)
              agvstat.work_state = AGV_WKSTATE_READY;
          break;
    ...
    agvcp_notify_event(evt, param_l, param);
    return 0;
}
...
static int issue_command(agv_action_t cmd, uint32_t l, const uint8_t *params)
{
      ...
      (cmd == ACTION_STOP_CHARGE) )
    {
         ...
         * */
        uint8_t action = cmd;
        motionctrl_event_callback(MOTIONCTRL_EVENT_ACTION_OVER, 1, &action);
        return PERR_EOK;
    }
    else if(cmd == ACTION_SYS_RESET)
    {
        APP_TRACE("system reset.\r\n");

        OSTimeDly(MS_TO_TICKS(200));
        system_reset();
    }
    ...
    if(ret == -PERR_EBUSY)
        APP_TRACE("Action is denied because of busy.\r\n");
...
static int check_and_release_point(uint32_t point)
{
    ...
    {
        APP_TRACE("check lock_point error! expect=%d actual=%d\r\n",
                  expect_point, point);
...
#if 0
void agv_error(uint8_t err, uint8_t*param, uint32_t param_len)
{
